...
def _should_avoid_tool_strategy(provider: str, model_name: str | None, cfg) -> bool:
    """判断是否应避免 ToolStrategy（避免发送 tool_choice 到不兼容模型）"""
    if provider != "openai":
        return False

    effective_model = (cfg.resolve_model_name(model_name) or "").lower()
    base_url = (getattr(cfg, "base_url", "") or "").lower()

    # DeepSeek 推理模型（如 deepseek-reasoner）不支持 tool_choice
    if "reasoner" in effective_model and "deepseek" in (effective_model + base_url):
        return True
    return False


def _invoke_with_json_schema_fallback(*, prompt: str, provider: str, model_name: str | None, temperature: float, schema, system_prompt: str):
    """不使用 ToolStrategy 的结构化调用兜底路径。"""
    ...
    try:
        # 对 OpenAI-compatible（含 DeepSeek）优先使用 json_mode，避免 tool_choice。
        return model.with_structured_output(schema, method="json_mode").invoke(messages)
    except TypeError:
        # 部分模型/版本不支持 method 参数。
...
def _invoke_structured(
    *,
    ...
    if (not cfg.supports_function_calling) or _should_avoid_tool_strategy(provider, model_name, cfg):
        if _should_avoid_tool_strategy(provider, model_name, cfg):
            effective = cfg.resolve_model_name(model_name)
            logger.info("模型 %s 不兼容 tool_choice，切换为 JSON 结构化兜底路径", effective)
        return _invoke_with_json_schema_fallback(
            prompt=prompt,
            provider=provider,
